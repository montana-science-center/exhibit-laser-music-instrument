...
uint8_t greenDetector = 0;
uint8_t redDetector = 0;
uint8_t greenDetectorPrev = 0;
uint8_t redDetectorPrev = 0;
...
//  }
  ...
  greenDetector = digitalRead(13);
  redDetector = digitalRead(12);

//  Serial.println(greenDetector);
//  Serial.println(redDetector);

  if (greenDetector != greenDetectorPrev) {
  ...
  if (redDetector != redDetectorPrev) {
    if (redDetector == 1) {
      midiNoteOn(0, 50, 80);
    }
    else {
  ...
  greenDetectorPrev = greenDetector;
  redDetectorPrev = redDetector;
